      ...
      <div className="relative max-w-2xl w-full text-center space-y-6">
        ...
        {showHow && (
          <div className="text-left border border-slate-800 rounded-lg bg-slate-900/70 p-5 text-sm text-slate-300 space-y-2">
            <p>Move through 10 security rooms. Each one holds a self-contained cybersecurity challenge.</p>
            <p>Read the clues, work out the answer, submit it. A correct answer recovers evidence and unlocks the next room.</p>
            <p>Wrong answers and hints raise the AEGIS alert level. The clock never stops: reach the Black Vault before the 30-minute purge completes.</p>
          </div>
        )}

        <div className="grid grid-cols-3 gap-3 max-w-md mx-auto pt-2">
          {[["10", "ROOMS"], ["30", "MINUTES"], ["1", "MISSION"]].map(([n, l]) => (
...
EVIDENCE RECOVERED:  ${state.collectedEvidence.length} / 10`
            : `AEGIS HAS COMPLETED THE PURGE.
ALL REMAINING EVIDENCE HAS BEEN DESTROYED.

MISSION STATUS:      FAILED
ROOMS COMPLETED:     ${state.completedRooms.length} / 10
EVIDENCE RECOVERED:  ${state.collectedEvidence.length} / 10`}
        </Mono>
