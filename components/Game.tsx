...
export default function Game() {
  ...
  if (screen === "landing")
    return <Landing onStart={newGame} hasSave={hasSave && state.gameStatus === "active"} onContinue={continueGame} />;
          ...
          <div className="ml-auto flex items-center gap-3 sm:gap-5">
            ...
            <div className="flex items-center gap-2 font-mono text-xs">
              ...
              <button onClick={() => setShowArchive(true)} className="hover:text-cyan-300 text-slate-300">
                {state.collectedEvidence.length}/10
              </button>
            </div>
            <div
        ...
        <aside className="hidden xl:block w-60 shrink-0 border-l border-slate-800 bg-slate-950/60 p-4 space-y-6">
          <div>
            ...
            <div className="space-y-2 text-xs font-mono text-slate-300">
              <div className="flex justify-between"><span className="text-slate-500">Rooms</span><span>{state.completedRooms.length}/10</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Evidence</span><span>{state.collectedEvidence.length}/10</span></div>
              ...
              <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden mt-1">
                <div className="h-full bg-cyan-400 transition-all duration-500" style={{ width: `${state.completedRooms.length * 10}%` }} />
              </div>
            </div>
          </div>
